...
class RaspberryPublicBootstrapContractTests(unittest.TestCase):
    ...
    def test_physical_deploy_no_longer_requires_missing_github_secrets(self):
        self.assertNotIn("secrets.ETR_REMOTE_GATEWAY_WSS", self.workflow)
        self.assertNotIn("secrets.FIREBASE_API_KEY", self.workflow)
        self.assertNotIn("ETR_REMOTE_GATEWAY_WSS est requis", self.workflow)
